...
- **Interaction Strategy**: For non-zero global trends, features must be interacted with group IDs (e.g., `time_idx * cluster`) so the model can learn segment-specific growth trajectories.
